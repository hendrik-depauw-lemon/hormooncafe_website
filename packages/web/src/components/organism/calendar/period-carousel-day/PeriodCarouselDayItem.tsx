...
export interface CarouselPeriodDayItemProps {
    data: Period;
    onPeriodSelect: (period: Period) => void;
    selected?: boolean;
}
export const CarouselPeriodDayItem = ({
    data,
    selected,
    onPeriodSelect,
}: CarouselPeriodDayItemProps) => {
    return (
        <Button
            className={cn(
        ...
        >
            <p className="text-base">{format(data.start, 'dd')}</p>
            <span className="text-[10px]">{format(data.start, 'eee')}</span>
        </Button>
    );
};
